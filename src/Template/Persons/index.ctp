...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Lista Familiares'), ['controller' => 'Details', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('Agregar Persona a Familia'), ['controller' => 'Details', 'action' => 'add']) ?></li>
    </ul>
</nav>
    ...
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('Ver'), ['action' => 'view', $person->id]) ?>
                    <?= $this->Html->link(__('Editar'), ['action' => 'edit', $person->id]) ?>
                    <?= $this->Form->postLink(__('Borrar'), ['action' => 'delete', $person->id], ['confirm' => __('Are you sure you want to delete # {0}?', $person->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
